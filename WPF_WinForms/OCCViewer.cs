    ...
    public class OCCViewer : System.Windows.Forms.Form
    {
        ...
        public void ReadFile()
        {
            ...
            try
            {
                ...
                //panels = new List<Panel>(numOfPlates);


                for (int i = 0; i < numOfPlates; i++)
                {

                    string plateName = sr.ReadLine();
                    string matName = sr.ReadLine();
                    string thickness = sr.ReadLine();

                    //string comp1 = sr.ReadLine();
                    //string comp2 = sr.ReadLine();

                    string vertTitle = sr.ReadLine();
                    while (vertTitle.Trim() != "Vertices:")
                    {
                        vertTitle = sr.ReadLine();
                    }
                    ...
                    {
                        string vertexLine = sr.ReadLine();
                        string[] vertexArr = System.Text.RegularExpressions.Regex.Split(vertexLine, "\\s{2,}");
                        (double, double, double) newPoint;
                        newPoint.Item1 = Convert.ToDouble(vertexArr[0]);
                        newPoint.Item2 = Convert.ToDouble(vertexArr[1]);
                        newPoint.Item3 = Convert.ToDouble(vertexArr[2]);

                int possibleExistingPointIndex = -1;

                        if (points.Reverse.TryGetValue(newPoint, out possibleExistingPointIndex))
                        {
                            if (possibleExistingPointIndex != -1)
                            {
                                pointIndexMap.Add(j, possibleExistingPointIndex);
                            }
                        }

                        if (!pointIndexMap.ContainsKey(j))
                        {
                            int newPointIndex = points.Count;
                            points.Add(newPointIndex, newPoint);
                            pointIndexMap.Add(j, newPointIndex);
                        }
                    }

                    string triaTitle = sr.ReadLine();
                    int triaCount = Convert.ToInt32(sr.ReadLine());
                    //var itriangles = new List<IndexTriangle>(triaCount);

                    for (int j = 0; j < triaCount; j++)
                    {
                        string triaLine = sr.ReadLine();
                        string[] triaArr = System.Text.RegularExpressions.Regex.Split(triaLine, "\\s{2,}");
                    ...
                    //var panel = new Panel();
                    //panel.Id = i;
                    //panel.Name = plateName;
                    ////panel.Vertices = points;
                    //panel.Triangles = itriangles;
